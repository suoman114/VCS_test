...
- 테스트마다 격리된 임시 SQLite DB를 쓴다. 기존 코드가 `SessionLocal`을
  ...
  app.services.mcptt.executor, app.services.mcptt.performance_executor,
  app.services.vcs_settings_store), 격리를 위해서는 이 7곳 전부를
  monkeypatch해야 한다(단순히 app.core.database.SessionLocal만 바꾸면
  나머지 모듈은 여전히 원래 프로세스 전역 DB를 본다 — 새 모듈을 추가할
  때마다 여기도 같이 갱신해야 한다는 뜻이다, 2026-07-30 McpttPerformanceExecutor
  추가 때 이걸 빠뜨려서 `TestRun not found after execution`/
  `DetachedInstanceError`로 처음 발견됨).
...
import app.services.mcptt.performance_executor as mcptt_performance_executor_module
...
def isolated_db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Callable[[], Session]:
    ...
    monkeypatch.setattr(mcptt_performance_executor_module, "SessionLocal", session_factory)
