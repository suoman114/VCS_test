...
- [x] **실 서버 장애: SSH 로그 tail이 예외/로그 하나 없이 조용히 멈추고 TestRun이 영원히 "running"에 머무는 문제** (2026-07-29 실 서버 backend.log로 근본 원인 확인) — `SSHConnector.connect()`(`backend/app/services/ssh_connector/client.py`)가 `asyncssh.connect()`를 타임아웃 없이 호출하고 있었다. asyncssh의 `login_timeout`(기본 120초)은 TCP 연결이 이미 수립된 뒤에야 시작되므로, TCP handshake 자체가 응답 없이 멈추는 경우(네트워크 순단 등)는 전혀 보호되지 않아 `connect()` 호출이 영원히 끝나지 않을 수 있었다. `SshTailSource`가 연결 끊김 후 재연결을 시도하다 바로 이 지점에서 멈추면, `VolteBasicCallExecutor.run()`의 `await session.stop()`(→ `asyncio.gather`로 해당 태스크를 기다림)도 영원히 끝나지 않아 최종 상태 반영(`persist_results`/`job_runner.set_status`)까지 도달하지 못했다 — `timeout_sec`(기본 30초)이 지나도 TestRun이 `running`에서 멈춰있던 증상과 정확히 일치. `Settings.vcs_ssh_connect_timeout_sec`(기본 15초)로 `asyncssh.connect()`를 `asyncio.wait_for`로 감싸 강제 타임아웃을 걸도록 수정(`SSHConnector.__init__`/`connect`). 타임아웃 시 명시적으로 로그를 남기고 `TimeoutError`를 raise하므로, `SshTailSource`의 기존 재시도/백오프 로직(최대 5회 후 `LogSourceError`)이 정상 작동해 무한 대기 대신 유한 시간 안에 실패·복구·에러 보고 중 하나로 귀결된다.
