...
- [ ] **(위 항목 후속, 2026-07-30 재확인)** 사용자가 최신 fix 배포 후에도 INVITE/BYE가 여전히 안 보인다고 재보고 — 그리고 실행 중 Call Flow 참가자 레인이 처음엔 VCTP/VCSM/VCMM(VoLTE)로 보였다가 몇 초 뒤 SIPp/UE/VCMC로 바뀌는 추가 증상도 확인됨. 두 번째 증상은 원인이 명확해 수정했다: `wait_for_completion`이 폴링 중 `persist_call_flow`를 호출할 때 `protocol=None`(자동판별)로 넘겨서, McPTT는 SIP 시그널링 소스가 vcmc_log 하나뿐이라 그 로그의 첫 이벤트가 파싱되기 전(vcmm_log의 RECORDING_* 이벤트만 있는 상태)엔 `detect_protocol`이 기본값 volte로 잘못 추측했다 — `wait_for_completion`/`persist_call_flow`에 `protocol` 매개변수를 추가해 volte/mcptt executor가 자신이 아는 프로토콜을 명시적으로 넘기도록 수정(`persist_results`는 원래도 명시적이었음). 회귀 테스트 추가(`test_persist_call_flow_uses_explicit_protocol_instead_of_guessing`). **INVITE/BYE 자체가 여전히 안 보이는 문제는 미해결** — `_edge_for_vcmc`가 이벤트를 버리지 않게 고친 이전 fix(바로 위 항목)로는 부족했다는 뜻은, vcmc.log의 SIP 메시지가 애초에 `VcmcLogAdapter`에서 `CallEvent`로 파싱조차 안 되고 있을 가능성이 높다는 신호다(RECORDING_* 메시지는 전부 vcmm_log 소스라 vcmc_log 파싱과 무관하게 정상 동작하는 것과 대비됨). `docs/log_samples/mcptt/vcmc.log`(2026-07-28 확보 샘플)로는 오프라인 재현 시 INVITE/BYE 포함 9개 SIP 이벤트가 전부 정상 파싱되는 것을 재확인했다 — 즉 파서 로직 자체는 이 샘플에 대해 올바르게 동작하므로, 실 서버의 실제 수집된 vcmc.log 내용이 이 샘플과 구조적으로 뭔가 다르다는 뜻이다. 사용자에게 실제 vcmc.log 원문(대시보드 "VCMC (SIP+MCPTT)" 탭 또는 SSH로 확인한 INVITE 블록 전체)을 요청해 정확한 차이를 찾아야 다음 단계를 진행할 수 있다.
